...
CRAWLER_NAME = "customer360_ingestion_crawler"
WORKFLOW_NAME = "Customer360_Ingestion"
GLUE_ROLE = "glue_dev_role"
RDS_CONNECTION_NAME = "my-rds-mysql-connection"
GLUE_DATABASE_NAME = "bronze_db"
S3_TARGET_PATH = [f"s3://{BUCKET_NAME}/analytics/ingestion/"]
table_prefix = "raw_"
PROJECT_LIB_PATH=f"s3://{BUCKET_NAME}/code/customer_analytics/customer_analytics-0.1.0-py3-none-any.whl"


GLUE_JOBS = {
    "ingestion_rds": f"s3://{BUCKET_NAME}/code/customer_analytics/ingestion_rds.py",
    "ingestion_dynamodb": f"s3://{BUCKET_NAME}/code/customer_analytics/ingestion_dynamodb.py"
}
...
def create_glue_job(job_name, script_path,role_arn, is_ingestion_job=False):
    ...
    job_args = {
        ...
        "DefaultArguments": {
            ...
            "--TempDir": f"s3://may-mani-2025-training-bucket/code/temp/{job_name}/",
